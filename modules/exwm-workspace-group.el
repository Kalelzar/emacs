...
(defun ewg/update-workspace-number ()
  "Set `ewg/display-workspace-number' to the current workspace number for display in the mode-line"
  (interactive)
  (setq ewg/display-workspace-number (format " [Workspace: %d]"
                                                         (ewg/get-group-index (or (exwm-workspace--position exwm-workspace--current) 0)))))

(defvar ewg/display-workspace-number "")

(add-to-list 'global-mode-string 'ewg/display-workspace-number t)
...
(defun ewg/init (monitor-names)
  ...
  (let ((xrandr-update
         (pcase ewg/max-group-size
           ...
           (_ (error "The layout is not implemented")))))
    (when xrandr-update
      (add-hook 'exwm-randr-screen-change-hook xrandr-update)))
  (exwm-randr-enable)
  (ewg/update-workspace-number))
    ...
    (defun ewg/xrandr-dual-monitor ()
      ...
      (start-process-shell-command
       ...
       (format "xrandr --output %s --auto \
               ...
               (nth 1 ewg/monitor-names) (nth 0 ewg/monitor-names)))
      (progn
