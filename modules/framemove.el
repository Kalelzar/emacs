...
(defun fm-other-workspace-in-group (count)
  (if-let* ((current-index exwm-workspace-current-index)
            (next-index (+ current-index count))
            (valid-next-index (>= next-index 0))
            (current-group (ewg/get-group-index current-index))
            (resulting-group (ewg/get-group-index next-index))
            (valid-resulting-group (= current-group resulting-group)))
      (exwm-workspace-switch next-index)
      (error "No workspace in that direction")))
...
(defun fm-next-frame (dir)
  ...
  (cond
   ((eql dir 'left) (fm-other-workspace-in-group 1))
   ((eql dir 'right) (fm-other-workspace-in-group -1))
   (t (let* ((thisframe (selected-frame))
