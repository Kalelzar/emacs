...
(defun show-selected-packages-not-in-config ()
  "Open a buffer containing all (not-dependency) packages not explicitly installed in config."
  (interactive)
  (with-current-buffer "*Missing Packages*"
    (kill-region (buffer-end -1) (buffer-end 1))
    (insert (s-join "\n" (get-selected-packages-not-in-config))))
  (view-buffer "*Missing Packages*" #'kill-buffer)
  (goto-char (point-min)))
#+end_src
