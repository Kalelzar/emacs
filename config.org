...
(require 'quelpa-use-package)(defun use-local-package (package &rest args)
...
Watch my [[https://melpa.org/#/activity-watch-mode][emacs activity]] with [[https://github.com/ActivityWatch/activitywatch/][ActivityWatch]]
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ;(add-to-list 'load-path "~/.emacs.d/elpa/org-link-beautify/")
...
#+begin_src emacs-lisp :tangle yes
    ...
    (defun org-mode-enable-minor-modes-hook ()
      ...
      (org-cdlatex-mode 1) ; Maybe don't keep this on at all times
                           ; Or change the keybinding from '`' since it is
                           ; sometimes used in emacs lisp
...
(setq org-todo-keywords
  '((sequence "NEW(n!)" "SCHEDULED(s!)" "IMMINENT(i!)" "ACTIVE(a!)" "|" "DONE(d!)" "CANCELLED(c!)")
...
#+begin_src emacs-lisp :tangle yes
(defvar kaltask-active-task nil
  "Stores the current active task.")

(defvar kaltask-task-file "~/.cache/activeTask"
  "File were the current `kaltask-active-task' is stored.")
(defvar kaltask-task-file-length-limited-suffix "Long"
  "The suffix of the file created if `kaltask-limit-task-length-in-file' is non-nil.")
(defvar kaltask-limit-task-length-in-file 30
  "If an int limit the length of the task name in `kaltask-task-file' to it.
In such a case a file containing the unaltered task name will be created
with the suffix `kaltask-task-file-length-limited-suffix' in the same directory.")

(defun kaltask-active-task-p ()
  "Check if `kaltask-active-task' is set."
  (not (null kaltask-active-task)))

(defun kaltask-get-active-task ()
  "Return the value of `kal-active-task' if non-nil, and 'None' otherwise."
  (if (kaltask-active-task-p)
      kaltask-active-task
    "None"))

(defun kaltask-update-task-file ()
  "Update the `kaltask-task-file' to match the current state of `kaltask-active-task'."
  (with-temp-file kaltask-task-file
    (let ((task (if (integerp kaltask-limit-task-length-in-file)
                    (substring (kaltask-get-active-task)
                               0
                               (min kaltask-limit-task-length-in-file
                                    (length (kaltask-get-active-task))))
                  (kaltask-get-active-task))))
      (delete-region (buffer-end -1)
                     (buffer-end 1))
      (insert task
              (if (< kaltask-limit-task-length-in-file
                       (length (kaltask-get-active-task)))
                   "..."
                ""))))

  (when (integerp kaltask-limit-task-length-in-file)
    (with-temp-file (concat kaltask-task-file
                            kaltask-task-file-length-limited-suffix)
      (delete-region (buffer-end -1)
                     (buffer-end 1))
      (insert (kaltask-get-active-task)))))

(defun kaltask-set-active-task (task)
  "Set the active TASK to any non-nil string.
This doesn't necessarily need to be a `org-mode' headline."
  (if (and (kaltask-active-task-p) (not (null task)))
       (error "There already is an active task")
     (progn
       (setq kaltask-active-task task)
       (kaltask-update-task-file))))

(defun kaltask-enforce-single-active ()
  "Enforce sole ACTIVE task."
  (let ((current-state org-state)
        (heading (substring-no-properties (org-get-heading t t t t))))
    (if (string= current-state "ACTIVE")
      (if (kaltask-active-task-p)
          (progn
            (message "There already is an active task.")
            (org-todo "IMMINENT"))
        (kaltask-set-active-task
         heading))
      (when (string= heading kaltask-active-task)
          (kaltask-set-active-task nil)))))

(add-hook 'org-after-todo-state-change-hook #'kaltask-enforce-single-active)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
;(add-to-list 'load-path "~/.emacs.d/elpa/novel-mode/")
;(use-package novel-mode)
(use-package novel-mode
 :quelpa (novel-mode :fetcher github :repo "TLINDEN/novel-mode"))
#+end_src
...
Set the inferior Lisp program for SLIME to [[https://sourceforge.net/projects/sbcl/][sbcl]].
#+begin_src emacs-lisp :tangle yes
